...
func withAtomic() {
	...
	for _ = range 70 {  
		go func() {
			defer wg.Done() //  Уменьшаем значение счётчика
			atomic.AddInt64(&c.v, 1)  //  Атомарно увеличиваем значение счетчика
		}()

	}
	wg.Wait()  //  Создаём joint point, чтобы дождаться выполнения всех горутин
